...
const Pages = {
  ...
  dashboard() {
    return `
      ...
      <div class="row g-3 mb-4" id="dash-stats">
        <div class="col-6 col-md-3"><div class="card stat-card p-3"><div class="stat-icon"><i class="bi bi-people-fill"></i></div><div class="stat-value" id="stat-students">--</div><div class="stat-label">\u05EA\u05DC\u05DE\u05D9\u05D3\u05D9\u05DD</div></div></div>
        <div class="col-6 col-md-3"><div class="card stat-card p-3"><div class="stat-icon"><i class="bi bi-calendar-check-fill"></i></div><div class="stat-value text-success" id="stat-attendance">--</div><div class="stat-label">\u05E0\u05D5\u05DB\u05D7\u05D5\u05EA \u05D4\u05D9\u05D5\u05DD</div></div></div>
        <div class="col-6 col-md-3"><div class="card stat-card p-3"><div class="stat-icon"><i class="bi bi-person-badge-fill"></i></div><div class="stat-value text-primary" id="stat-staff">--</div><div class="stat-label">\u05D0\u05E0\u05E9\u05D9 \u05E6\u05D5\u05D5\u05EA</div></div></div>
        <div class="col-6 col-md-3"><div class="card stat-card p-3"><div class="stat-icon"><i class="bi bi-cash-stack"></i></div><div class="stat-value text-warning" id="stat-debt">--</div><div class="stat-label">\u05D7\u05D5\u05D1\u05D5\u05EA \u05E4\u05EA\u05D5\u05D7\u05D9\u05DD</div></div></div>
      </div>
      <div class="row g-3">
        <div class="col-lg-8"><div class="card p-3"><h6 class="fw-bold mb-3"><i class="bi bi-bar-chart-fill me-2 text-primary"></i>\u05E0\u05D5\u05DB\u05D7\u05D5\u05EA \u05D4\u05D9\u05D5\u05DD</h6><div class="chart-container" style="height:260px"><canvas id="chart-attendance"></canvas></div></div></div>
        <div class="col-lg-4">
          <div class="card p-3"><h6 class="fw-bold mb-3"><i class="bi bi-lightning-fill me-2 text-warning"></i>\u05E4\u05E2\u05D5\u05DC\u05D5\u05EA \u05DE\u05D4\u05D9\u05E8\u05D5\u05EA</h6>
            <div class="d-grid gap-2">
              <a href="#attendance" class="btn btn-outline-primary d-flex align-items-center gap-2"><i class="bi bi-calendar-check"></i>\u05E1\u05DE\u05DF \u05E0\u05D5\u05DB\u05D7\u05D5\u05EA</a>
              <a href="#students" class="btn btn-outline-success d-flex align-items-center gap-2"><i class="bi bi-person-plus"></i>\u05E6\u05E4\u05D4 \u05D1\u05EA\u05DC\u05DE\u05D9\u05D3\u05D9\u05DD</a>
              <a href="#tasks" class="btn btn-outline-info d-flex align-items-center gap-2"><i class="bi bi-kanban"></i>\u05DE\u05E9\u05D9\u05DE\u05D5\u05EA</a>
              <a href="#finance" class="btn btn-outline-warning d-flex align-items-center gap-2"><i class="bi bi-cash"></i>\u05DB\u05E1\u05E4\u05D9\u05DD</a>
            </div>
          </div>
          <div class="card p-3 mt-3"><h6 class="fw-bold mb-3"><i class="bi bi-pie-chart-fill me-2 text-success"></i>\u05DE\u05E6\u05D1 \u05DB\u05E1\u05E4\u05D9</h6><div class="chart-container" style="height:200px"><canvas id="chart-finance"></canvas></div></div>
        </div>
      </div>
      <div class="card p-3 mt-3"><h6 class="fw-bold mb-3"><i class="bi bi-clock-history me-2 text-info"></i>\u05E4\u05E2\u05D9\u05DC\u05D5\u05EA \u05D0\u05D7\u05E8\u05D5\u05E0\u05D5\u05EA</h6><div id="activity-feed"><div class="text-muted text-center py-3">\u05D8\u05D5\u05E2\u05DF \u05E0\u05EA\u05D5\u05E0\u05D9\u05DD...</div></div></div>
    `;
  },
  ...
  async dashboardInit() {
    ...
    if (attCtx) {
      App.charts.att = new Chart(attCtx, { type: 'bar', data: { labels: ['\u05E0\u05D5\u05DB\u05D7', '\u05D7\u05D9\u05E1\u05D5\u05E8', '\u05D0\u05D9\u05D7\u05D5\u05E8'], datasets: [{ data: [present.length, absent.length, late.length], backgroundColor: ['#0f9d58', '#ea4335', '#f9ab00'], borderRadius: 8, barPercentage: 0.6 }] }, options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { y: { beginAtZero: true, ticks: { stepSize: 1 } }, x: { grid: { display: false } } } } });
    }
    ...
    feed.innerHTML = activities.map(a => `<div class="d-flex align-items-center gap-3 py-2 border-bottom"><div class="avatar avatar-sm" style="background:var(--bht-${a.color},#6c757d)"><i class="bi bi-${a.icon}" style="font-size:.8rem"></i></div><div class="flex-grow-1"><span>${a.text}</span></div><small class="text-muted">${a.time}</small></div>`).join('');
  },
  ...
  async studentInit(id) {
    ...
    document.getElementById('student-card-content').innerHTML = `
      ...
      <ul class="nav nav-tabs-bht mb-3" role="tablist">
        ...
        <li class="nav-item"><a class="nav-link" data-bs-toggle="tab" href="#tab-beh">\u05D4\u05EA\u05E0\u05D4\u05D2\u05D5\u05EA</a></li>
        <li class="nav-item"><a class="nav-link" data-bs-toggle="tab" href="#tab-grades">\u05E6\u05D9\u05D5\u05E0\u05D9\u05DD</a></li>
      </ul>
      <div class="tab-content">
        ...
        `<div class="card"><table class="table table-bht mb-0"><thead><tr><th>\u05EA\u05D0\u05E8\u05D9\u05DA</th><th>\u05E1\u05D8\u05D8\u05D5\u05E1</th></tr></thead><tbody>${studentAtt.slice(-10).reverse().map(a => `<tr><td>${Utils.formatDateShort(a['\u05EA\u05D0\u05E8\u05D9\u05DA'])}</td><td>${a['\u05E1\u05D8\u05D8\u05D5\u05E1'] === '\u05E0\u05D5\u05DB\u05D7' ? '<span class="badge bg-success">\u05E0\u05D5\u05DB\u05D7</span>' : '<span class="badge bg-danger">\u05D7\u05D9\u05E1\u05D5\u05E8</span>'}</td></tr>`).join('')}</tbody></table></div>`}</div>
        <div class="tab-pane fade" id="tab-fin"><div class="card p-3"><div class="row g-3 text-center mb-3">
          <div class="col-4"><div class="fs-5 fw-bold">${Utils.formatCurrency(sf['\u05E1\u05DB\u05D5\u05DD'] || 0)}</div><small class="text-muted">\u05E1\u05D4"\u05DB</small></div>
          <div class="col-4"><div class="fs-5 fw-bold text-success">${Utils.formatCurrency(sf['\u05E9\u05D5\u05DC\u05DD'] || 0)}</div><small class="text-muted">\u05E9\u05D5\u05DC\u05DD</small></div>
          <div class="col-4"><div class="fs-5 fw-bold text-danger">${Utils.formatCurrency(sf['\u05D9\u05EA\u05E8\u05D4'] || 0)}</div><small class="text-muted">\u05D9\u05EA\u05E8\u05D4</small></div>
        </div>${sf['\u05E1\u05DB\u05D5\u05DD'] ? `<div class="finance-progress"><div class="finance-progress-bar bg-success" style="width:${Math.round((sf['\u05E9\u05D5\u05DC\u05DD']||0)/(sf['\u05E1\u05DB\u05D5\u05DD']||1)*100)}%"></div></div><small class="text-muted mt-1 d-block">${Math.round((sf['\u05E9\u05D5\u05DC\u05DD']||0)/(sf['\u05E1\u05DB\u05D5\u05DD']||1)*100)}% \u05E9\u05D5\u05DC\u05DD</small>` : '<div class="text-muted text-center">\u05D0\u05D9\u05DF \u05E0\u05EA\u05D5\u05E0\u05D9 \u05DB\u05E1\u05E4\u05D9\u05DD</div>'}</div></div>
        <div class="tab-pane fade" id="tab-beh"><div class="d-flex gap-3 mb-3"><span class="badge bg-success p-2">+${posB} \u05D7\u05D9\u05D5\u05D1\u05D9</span><span class="badge bg-danger p-2">-${negB} \u05E9\u05DC\u05D9\u05DC\u05D9</span></div>
          ${studentBeh.length === 0 ? '<div class="text-muted text-center">\u05D0\u05D9\u05DF \u05D3\u05D9\u05D5\u05D5\u05D7\u05D9 \u05D4\u05EA\u05E0\u05D4\u05D2\u05D5\u05EA</div>' :
          `<table class="table table-sm"><thead><tr><th>\u05EA\u05D0\u05E8\u05D9\u05DA</th><th>\u05E1\u05D5\u05D2</th><th>\u05EA\u05D9\u05D0\u05D5\u05E8</th></tr></thead><tbody>${studentBeh.slice(-10).reverse().map(b => `<tr><td>${Utils.formatDateShort(b['\u05EA\u05D0\u05E8\u05D9\u05DA'])}</td><td><span class="badge bg-${b['\u05E1\u05D5\u05D2']==='\u05D7\u05D9\u05D5\u05D1\u05D9'?'success':'danger'}">${b['\u05E1\u05D5\u05D2']||''}</span></td><td>${b['\u05EA\u05D9\u05D0\u05D5\u05E8']||''}</td></tr>`).join('')}</tbody></table>`}</div>
        <div class="tab-pane fade" id="tab-grades"><div class="text-muted text-center py-3">\u05D8\u05D5\u05E2\u05DF \u05E6\u05D9\u05D5\u05E0\u05D9\u05DD...</div></div>
      </div>
    `;
  },
  ...
  renderHw() {
    ...
    document.getElementById('hw-cards').innerHTML = this._hwData.slice().reverse().map(r => {
      ...
      return `<div class="col-md-6"><div class="card p-3 ${overdue?'border-danger':''}" style="border-width:2px"><div class="d-flex justify-content-between mb-2"><div><span class="badge bg-primary me-1">${r['\u05E1\u05D5\u05D2']||r['\u05DE\u05E7\u05E6\u05D5\u05E2']||''}</span><span class="badge bg-secondary">${r['\u05DB\u05D9\u05EA\u05D4']||''}</span></div>${daysLeft!==null?`<span class="badge ${overdue?'bg-danger':daysLeft<=2?'bg-warning':'bg-success'}">${overdue?'\u05E2\u05D1\u05E8 \u05DE\u05D5\u05E2\u05D3!':daysLeft===0?'\u05D4\u05D9\u05D5\u05DD!':daysLeft+' \u05D9\u05DE\u05D9\u05DD'}</span>`:''}</div><h6 class="fw-bold">${r['\u05DE\u05E7\u05E6\u05D5\u05E2']||''}</h6><p class="small mb-1">${r['\u05EA\u05D9\u05D0\u05D5\u05E8']||''}</p><div class="small text-muted"><i class="bi bi-calendar me-1"></i>\u05E0\u05D9\u05EA\u05DF: ${r['\u05EA\u05D0\u05E8\u05D9\u05DA_\u05DE\u05EA\u05DF']||''} ${due?' | \u05D4\u05D2\u05E9\u05D4: '+due:''}</div></div></div>`;
    }).join('');
  },
  ...
  renderPc() {
    ...
    let bal=0; document.getElementById('pc-list').innerHTML = `<div class="card"><table class="table table-bht mb-0"><thead><tr><th>\u05EA\u05D0\u05E8\u05D9\u05DA</th><th>\u05E1\u05D5\u05D2</th><th>\u05EA\u05D9\u05D0\u05D5\u05E8</th><th>\u05E1\u05DB\u05D5\u05DD</th><th>\u05D9\u05EA\u05E8\u05D4</th></tr></thead><tbody>${this._pcData.map(r => { const a=parseFloat(r['\u05E1\u05DB\u05D5\u05DD'])||0; if (r['\u05E1\u05D5\u05D2']==='\u05D4\u05DB\u05E0\u05E1\u05D4') bal+=a; else bal-=a; return `<tr><td>${r['\u05EA\u05D0\u05E8\u05D9\u05DA']||''}</td><td><span class="badge ${r['\u05E1\u05D5\u05D2']==='\u05D4\u05DB\u05E0\u05E1\u05D4'?'bg-success':'bg-danger'}">${r['\u05E1\u05D5\u05D2']}</span></td><td>${r['\u05EA\u05D9\u05D0\u05D5\u05E8']||''}</td><td class="fw-bold ${r['\u05E1\u05D5\u05D2']==='\u05D4\u05DB\u05E0\u05E1\u05D4'?'text-success':'text-danger'}">${r['\u05E1\u05D5\u05D2']==='\u05D4\u05DB\u05E0\u05E1\u05D4'?'+':'-'}\u20AA${a}</td><td class="fw-bold">\u20AA${bal}</td></tr>`; }).join('')}</tbody></table></div>`;
  },
  ...
  renderTrips() {
    ...
    document.getElementById('trip-cards').innerHTML = this._tripData.map(t => `<div class="col-md-6"><div class="card p-3"><div class="d-flex justify-content-between"><h6 class="fw-bold"><i class="bi bi-geo-alt-fill text-primary me-1"></i>${t['\u05D9\u05E2\u05D3']||''}</h6><span class="badge bg-${stC[t['\u05E1\u05D8\u05D8\u05D5\u05E1']]||'secondary'}">${t['\u05E1\u05D8\u05D8\u05D5\u05E1']||''}</span></div><div class="small text-muted"><i class="bi bi-calendar me-1"></i>${t['\u05EA\u05D0\u05E8\u05D9\u05DA_\u05D4\u05EA\u05D7\u05DC\u05D4']||''} ${t['\u05EA\u05D0\u05E8\u05D9\u05DA_\u05E1\u05D9\u05D5\u05DD']?' - '+t['\u05EA\u05D0\u05E8\u05D9\u05DA_\u05E1\u05D9\u05D5\u05DD']:''}</div>${t['\u05EA\u05D9\u05D0\u05D5\u05E8']?`<p class="small mb-0 mt-1">${t['\u05EA\u05D9\u05D0\u05D5\u05E8']}</p>`:''}</div></div>`).join('');
  },
  ...
  renderBudget() {
    ...
    let cum=0; document.getElementById('budg-list').innerHTML = `<div class="card"><table class="table table-bht mb-0"><thead><tr><th>\u05EA\u05D0\u05E8\u05D9\u05DA</th><th>\u05E7\u05D8\u05D2\u05D5\u05E8\u05D9\u05D4</th><th>\u05EA\u05D9\u05D0\u05D5\u05E8</th><th>\u05E1\u05DB\u05D5\u05DD</th><th>\u05DE\u05E6\u05D8\u05D1\u05E8</th><th>\u05E1\u05E4\u05E7</th></tr></thead><tbody>${this._budgData.map(r => { const a=parseFloat(r['\u05E1\u05DB\u05D5\u05DD'])||0; cum+=a; return `<tr><td>${r['\u05EA\u05D0\u05E8\u05D9\u05DA']||''}</td><td><span class="badge bg-secondary">${r['\u05E7\u05D8\u05D2\u05D5\u05E8\u05D9\u05D4']||''}</span></td><td>${r['\u05EA\u05D9\u05D0\u05D5\u05E8']||''}</td><td class="fw-bold text-danger">${Utils.formatCurrency(a)}</td><td class="fw-bold">${Utils.formatCurrency(cum)}</td><td>${r['\u05E1\u05E4\u05E7']||''}</td></tr>`; }).join('')}</tbody></table></div>`;
  },
  ...
  /* ======================================================================
     ...
     ====================================================================== */
  reports() {
    return `<div class="page-header d-flex justify-content-between align-items-start flex-wrap gap-2"><div><h1><i class="bi bi-file-earmark-bar-graph me-2"></i>\u05D3\u05D5\u05D7\u05D5\u05EA</h1></div><button class="btn btn-outline-secondary btn-sm" onclick="window.print()"><i class="bi bi-printer me-1"></i>\u05D4\u05D3\u05E4\u05E1</button></div><ul class="nav nav-pills mb-3"><li class="nav-item"><a class="nav-link active" href="#" onclick="Pages.loadReport('all',this)">\u05E1\u05E7\u05D9\u05E8\u05D4 \u05DB\u05DC\u05DC\u05D9\u05EA</a></li><li class="nav-item"><a class="nav-link" href="#" onclick="Pages.loadReport('attendance',this)">\u05E0\u05D5\u05DB\u05D7\u05D5\u05EA</a></li><li class="nav-item"><a class="nav-link" href="#" onclick="Pages.loadReport('finance',this)">\u05DB\u05E1\u05E4\u05D9\u05DD</a></li></ul><div id="rpt-content">${Utils.skeleton(3)}</div>`;
  },
  async reportsInit() { this.loadReport('all'); },
  async loadReport(type, el) {
    if (el) { document.querySelectorAll('.nav-pills .nav-link').forEach(a=>a.classList.remove('active')); el.classList.add('active'); }
    ...
    const students = await App.getData('\u05EA\u05DC\u05DE\u05D9\u05D3\u05D9\u05DD');
    const att = await App.getData('\u05E0\u05D5\u05DB\u05D7\u05D5\u05EA');
    const fin = await App.getData('\u05E9\u05DB\u05E8_\u05DC\u05D9\u05DE\u05D5\u05D3');
    const beh = await App.getData('\u05D4\u05EA\u05E0\u05D4\u05D2\u05D5\u05EA');
    let html = '';
    if (type==='all'||type==='attendance') {
      ...
      html+=`<div class="card p-3 mb-3"><h6 class="fw-bold"><i class="bi bi-clipboard-data me-2"></i>\u05E0\u05D5\u05DB\u05D7\u05D5\u05EA</h6><div class="row g-3 text-center"><div class="col-md-4"><div class="fs-3 fw-bold text-success">${pct}%</div><small>\u05D0\u05D7\u05D5\u05D6 \u05E0\u05D5\u05DB\u05D7\u05D5\u05EA</small></div><div class="col-md-4"><div class="fs-3 fw-bold text-primary">${present}</div><small>\u05E0\u05D5\u05DB\u05D7\u05D9\u05DD</small></div><div class="col-md-4"><div class="fs-3 fw-bold text-danger">${absent}</div><small>\u05D7\u05D9\u05E1\u05D5\u05E8\u05D9\u05DD</small></div></div></div>`;
    }
