...
Object.assign(Pages, {
  ...
  camerasInit() {
    ...
    if (!document.querySelector('.camera-feed')) return; // off-LAN placeholder rendered, no feeds to refresh
    this._camInterval = setInterval(() => {
      if (App.currentPage !== 'cameras') { clearInterval(this._camInterval); this._camInterval = null; return; }
      document.querySelectorAll('.camera-feed img').forEach(img => { const src = img.src.split('?')[0]; img.src = src + '?t=' + Date.now(); });
    }, 10000);
  },
