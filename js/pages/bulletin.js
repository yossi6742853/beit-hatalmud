...
Object.assign(Pages, {
  ...
  bulletinTogglePin(id) {
    ...
    if (b) {
      ...
      App.apiCall('update', 'לוח_מודעות', { id, row: b }).catch(e => console.warn('API error:', e));
      this._bulletinRender();
  ...
  saveBulletin() {
    ...
    if (this._blnEditId) {
      ...
      this._blnSaveToStorage();
      App.apiCall('update', 'לוח_מודעות', { id: this._blnEditId, row: b }).catch(e => console.warn('API error:', e));
      ...
      this._blnSaveToStorage();
      App.apiCall('add', 'לוח_מודעות', { row: newItem }).catch(e => console.warn('API error:', e));
  ...
  async deleteBulletin(id) {
    ...
    this._blnSaveToStorage();
    App.apiCall('delete', 'לוח_מודעות', { id }).catch(e => console.warn('API error:', e));
    this._bulletinRender();
